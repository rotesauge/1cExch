...
Процедура ОбработчикПриИзмененииНаСервере()
	HTML = "<html><body>" + РаскраситьКод1С8(Объект.Обработчик) + "</html></body>";
...
//Реализует функционал "Оформить фрагмент кода" - для соответствующих кнопок в формах (н-р: для поля "Описание" в форме док. "Задание")
&НаСервере
...
Процедура ОбработчикОповещения_Результат(Параметр1, Параметр2) Экспорт
	...
	Объект.Обработчик=Параметр1;
	HTML = "<html><body>" + РаскраситьКод1С8(Объект.Обработчик) + "</html></body>";
КонецПроцедуры

&НаКлиенте
Процедура РедакторКода(Команда)
	ПараметрыФормы=Новый Структура;
	ПараметрыФормы.Вставить("Алгоритм", Объект.Обработчик);
...
&НаКлиенте
Процедура РедакторКода1(Команда)
	ПараметрыФормы=Новый Структура;
	ПараметрыФормы.Вставить("Алгоритм", Объект.ОбработчикДобавления);
	ОткрытьФорму("Обработка.РедакторКода.Форма.Форма", ПараметрыФормы, ЭтаФорма, УникальныйИдентификатор,,, Новый ОписаниеОповещения("ОбработчикОповещения_Результат1", ЭтотОбъект),РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры

&НаКлиенте
Процедура ОбработчикОповещения_Результат1(Параметр1, Параметр2) Экспорт
	Если Параметр1=Неопределено Тогда Возврат; КонецЕсли;	
	Объект.ОбработчикДобавления=Параметр1;
	HTML1 = "<html><body>" + РаскраситьКод1С8(Объект.ОбработчикДобавления) + "</html></body>";
КонецПроцедуры
